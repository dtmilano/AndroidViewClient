...
class Device:
    ...
    def factory(_str):
        if DEBUG:
            ...
            print("   _str="+ str(repr(_str)),file=sys.stderr)
            print("   _str="+ _str.replace(' ', '_'),file=sys.stderr)
        values = _str.split(None, 2)
        if DEBUG:
            print("values=" + str(values),file=sys.stderr)
...
class WifiManager:
    ...
    def getWifiState(self):
        ...
        print("UNKNOWN WIFI STATE:"+str(state),file=sys.stderr)
...
class AdbClient:
    ...
    def __timeoutHandler(self, timerId, description=None):
        if DEBUG:
            print("\nTIMEOUT HANDLER "+str(timerId)+' : '+str(description),file=sys.stderr)
    ...
    def cancelTimer(self, timerId):
        ...
        if timerId not in self.timers:
            ...
            for t in self.timers:
                print("   id="+str(t),file=sys.stderr)
    ...
    def close(self):
        if DEBUG:
            print("Closing socket... "+str(self.socket),file=sys.stderr)
        if self.socket:
            self.socket.close()
    ...
    def __receive(self, nob=None, sock=None):
        ...
        try:
            if nob is None:
                nob = int(sock.recv(4).decode(), 16)
            if DEBUG:
                print("    __receive: receiving "+str(nob)+" bytes",file=sys.stderr)
            recv = bytearray(nob)
            ...
            while nr < nob:
                ...
                if DEBUG:
                    print("l= "+str(l)+" nr= "+str(nr),file=sys.stderr)
                    print("timer="+str(self.timers[timerId]),file=sys.stderr)
                view = view[l:]
                nr += l
                if self.timers[timerId] == 'EXPIRED':
    ...
    def __checkOk(self, sock=None):
        ...
        try:
            recv = sock.recv(4).decode()
        finally:
            self.cancelTimer(timerId)

        if DEBUG:
            print("    __checkOk: recv= "+str(repr(recv)),file=sys.stderr)
    ...
    def __setTransport(self, timeout=60):
        ...
        if len(devices) == 0 and timeout > 0:
            ...
            try:
                ...
                while not found:
                    ...
                    sys.stderr.flush()
                    try:
                        for line in _s.recv(1024).decode().splitlines():
        ...
        if DEBUG:
            print("    __setTransport: msg= "+str(msg),file=sys.stderr)
    ...
    def shell(self, _cmd=None, raw=False):
        if DEBUG:
        ...
        if USE_ADB_FRAMEBUFFER_METHOD:
            ...
            if DEBUG:
                if version == 1:
                    ...
                    print("    takeSnapshot: unknown version "+str(version),file=sys.stderr)
            ...
            try:
                ...
                raise RuntimeError('Cannot convert stream to image')
    ...
    def __transformPointByOrientation(self, xy, orientationOrig, orientationDest):
        x, y = xy
    ...
    def touch(self, x, y, orientation=-1, eventType=DOWN_AND_UP):
        if DEBUG_TOUCH:
            print("touch(x="+str(x)+", y="+str(y)+", orientation="+str(orientation)+", eventType="+str(eventType)+")",file=sys.stderr)
        self.__checkTransport()
        if orientation == -1:
    ...
    def touchDip(self, x, y, orientation=-1, eventType=DOWN_AND_UP):
        if DEBUG_TOUCH:
            print("touchDip(x= "+str(x)+", y= "+str(y)+", orientation= "+str(orientation)+", eventType= "+str(eventType)+" )",file=sys.stderr)
        self.__checkTransport()
        if orientation == -1:
            orientation = self.display['orientation']
    ...
    def drag(self, x0y0, x1y1, duration, steps=1, orientation=-1):
        """
    ...
    def dragDip(self, x0y0, x1y1, duration, steps=1, orientation=-1):
        """
    ...
    def log(self, tag, message, priority='D', verbose=False):
        ...
        if verbose or priority == 'V':
            print(tag + ':'+str(message),file=sys.stderr)
    ...
    def getWindows(self):
        ...
        if DEBUG_WINDOWS: print(str(dww),file=sys.stderr)
            ...
            if m:
                ...
                for l2 in range(l + 1, len(lines)):
                    ...
                    if m:
                        ...
                        if DEBUG_COORDS: print("getWindows: visibility="+str(visibility),file=sys.stderr)
                    ...
                    # print("Searching policyVisibility in "+str(lines[l2]),file=sys.stderr)
        ...
        if currentFocus in windows and windows[currentFocus].visibility == 0:
            if DEBUG_COORDS:
                print("getWindows: focus="+str(currentFocus),file=sys.stderr)
                print("getWindows:"+str(windows[currentFocus]),file=sys.stderr)
...
if __name__ == '__main__':
    ...
    if INTERACTIVE:
        ...
        print('date: '+str(adbClient.shell('date')))
